...
/// @notice The setup contract of the `L2TokenVoting` plugin.
...
contract L2TokenVotingSetup is PluginSetup {
    ...
    /// @notice The address of the `L2TokenVoting` base contract.
    L2TokenVoting private immutable tokenVotingBase;
    ...
    constructor(
        ...
        tokenVotingBase = new L2TokenVoting();
    ...
    function prepareInstallation(
        ...
        // Decode `_data` to extract the params needed for deploying and initializing `L2TokenVoting` plugin,
        // and the required helpers
        (
            L2MajorityVotingBase.VotingSettings memory votingSettings,
        ...
        plugin = createERC1967Proxy(
            ...
            abi.encodeCall(
                L2TokenVoting.initialize,
                (IDAO(_dao), votingSettings, IVotesUpgradeable(token), bridgeDAOSettings)
            )
        );
